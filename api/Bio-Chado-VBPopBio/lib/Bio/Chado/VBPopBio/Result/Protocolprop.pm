package Bio::Chado::VBPopBio::Result::Protocolprop;

use feature 'switch';
use base 'Bio::Chado::Schema::Result::NaturalDiversity::NdProtocolprop';
__PACKAGE__->load_components(qw/+Bio::Chado::VBPopBio::Util::Subclass/);
__PACKAGE__->subclass({ nd_protocol => 'Bio::Chado::VBPopBio::Result::Protocol',
			type => 'Bio::Chado::VBPopBio::Result::Cvterm',
		      });

=head1 NAME

Bio::Chado::VBPopBio::Result::Protocolprop
...
sub as_data_for_jsonref {
  my ($self, $seen) = @_;
  my $id = 'prp'.$self->nd_protocolprop_id;
  if ($seen->{$id}++) {
    return { '$ref' => $id };
  } else {
    return {
